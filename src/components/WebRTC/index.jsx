...
const Player = ({ size }) => {
  ...
  const { peerConnection, websocket, connected, audioTrack, videoTrack } =
    useSelector((state) => state.webrtcPlay);
  ...
  useEffect(() => {
    if (playSettings.playStart && !playSettings.playStarting && !connected) {
      dispatch({ type: SET_PLAY_FLAGS, playStart: false, playStarting: true });
      startPlay(playSettings, websocket, {
        onError: (error) => {
          dispatch({ type: SET_ERROR_MESSAGE, message: error.message });
        },
        onConnectionStateChange: (result) => {
          dispatch({
            type: SET_WEBRTC_PLAY_CONNECTED,
            connected: result.connected,
          });
        },
        onSetPeerConnection: (result) => {
          dispatch({
            type: SET_WEBRTC_PLAY_PEERCONNECTION,
            peerConnection: result.peerConnection,
          });
        },
        onSetWebsocket: (result) => {
          dispatch({
            type: SET_WEBRTC_PLAY_WEBSOCKET,
        ...
        onPeerConnectionOnTrack: (event) => {
          if (event.track != null && event.track.kind != null) {
            if (event.track.kind === "audio") {
              dispatch({
                type: SET_WEBRTC_PLAY_AUDIO_TRACK,
                audioTrack: event.track,
              });
            } else if (event.track.kind === "video") {
              dispatch({
                type: SET_WEBRTC_PLAY_VIDEO_TRACK,
    ...
    if (playSettings.playStarting && connected) {
      dispatch({ type: SET_PLAY_FLAGS, playStarting: false });
    }

    if (playSettings.playStop && !playSettings.playStopping && connected) {
      dispatch({ type: SET_PLAY_FLAGS, playStop: false, playStopping: true });
      stopPlay(peerConnection, websocket, {
        ...
        onPlayStopped: () => {
          dispatch({ type: SET_WEBRTC_PLAY_CONNECTED, connected: false });
        },
      });
    }
    if (playSettings.playStopping && !connected) {
      dispatch({ type: SET_PLAY_FLAGS, playStopping: false });
    }
  }, [
    dispatch,
    videoElement,
